...
folder_paths = {
    "Setup error: Hips are too low (like a squat)": "/users/eleves-a/2022/amine.chraibi/AI-Deadlift-Analysis/processed_videos/Setup error: Hips are too low (like a squat)",
    "Setup error: Rounded upper back (thoracic spine). Maintain a neutral spine.": "/users/eleves-a/2022/amine.chraibi/AI-Deadlift-Analysis/processed_videos/Setup error: Rounded upper back (thoracic spine). Maintain a neutral spine.", 
    "Setup phase is correct.": "/users/eleves-a/2022/amine.chraibi/AI-Deadlift-Analysis/processed_videos/Setup phase is correct.", 
}
    ...
    def process_video(self, file_path, label_idx):
        ...
        while True:
            ...
            if keypoints.shape[1] < num_joints:
                padding = torch.zeros((keypoints.shape[0],num_joints - keypoints.shape[1],keypoints.shape[2]), device=device)
...
# Model Configuration
num_keypoints = 36  # e.g., 17 joints with x, y coordinates + 2 barbell coordinates
...
best_model_path = "best_deadlift_setup_phase_classifier.pth"
best_loss = 1000000
# Training loop
